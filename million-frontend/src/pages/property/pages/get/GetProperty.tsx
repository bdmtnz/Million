...
const GetProperty = () => {    
    ...
    const handleSubmit = () => {
        if (page !== 1) {
            ...
            PropertyService.get(filter.keyword, pageSize, 1, filter.min, filter.max).then(response => {
                setProperties(response.data.page)
                setTotal(response.data.total)
            })
        }
                            ...
                            <div className="w-full flex justify-between gap-2">
                                <div className="w-full">
                                    ...
                                    <fieldset className="bg-gray-700 rounded-md p-1 gap-1 grid grid-cols-2">
                                        <InputNumber 
                                            value={filter.min}
                                            onChange={(e) => setFilter({ ...filter, min: e.value ?? null })}
                                            mode="currency" 
                                            currency="USD"
                                            placeholder="Min"
                                            size={1}
                                        ...
                                        <InputNumber 
                                            value={filter.max} 
                                            onChange={(e) => setFilter({ ...filter, max: e.value ?? null })} 
                                            mode="currency" 
                                            currency="USD" 
                                            placeholder="Max"
                                            size={1}
